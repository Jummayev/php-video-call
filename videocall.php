...
if (isset($_GET['username']) && !empty($_GET['username'])){
    $username = \MyApp\FormSanitizer::sanitizeFromUsername($_GET['username']);
...
<header class="g-header">
    <div class="site-logo">
        ...
        <svg width="130px" height="30px" fill="#fff" version="1.1" xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="u-p-id" data-userId="<?= $userData->id; ?>" data-profileid="">

    </div>
    <div class="g-header-right">
        <span class="user-status online"><?= $userData->online_status ?></span>
        <div class="user-wrapper">
            <img src="<?= url_for($userData->avatar) ?>" alt="<?= url_for($userData->username) ?>">
        </div>
        <a href="<?= url_for("logout.php") ?>" class="log-out-status">Logout</a>
...
21 -159 58 -185 113 l-20 42 -63 1 -64 0 7 -32z"/>
            ...
            <div class="nf-2 custom-style active">
                <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="call-hang-status">
                    <div class="calling-status-wrap" id="#conf-int">
                        <div class="user-connected-img">
                            <img src="<?= url_for($profileData->avatar)?>" alt="<?= $profileData->username?>">
                        </div>
                        <div class="user-status-text">
                            <div class="user-calling-status">
                                Make a Call with
                            </div>
                            <div class="user-connected-name">
								<?= $profileData->username?>
                            </div>
                        </div>
                        <div class="calling-action">
                            <div class="video-call" data-profileid = "<?= $profileData->id ?>" data-userid = "<?= $userData->id ?>">
                                <svg  class="cam-icon" xmlns="http://www.w3.org/2000/svg" focusable="false" viewBox="0 0 24 24"><path d="M18 10.48V6c0-1.1-.9-2-2-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2v-4.48l4 3.98v-11l-4 3.98zm-2-.79V18H4V6h12v3.69z"/></svg>
                            </div>
                            <div class="audio-call">
                                <i class="fa fa-phone  audio-icon-top" data-profileid = "<?= $profileData->id ?>" data-userid = "<?= $userData->id ?>"></i>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    let conn = new WebSocket('ws://localhost:8080/webrtc/?token=<?= $userData->session_id?>');
</script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js"
        integrity="sha512-894YE6QWD5I59HgZOGReFYm4dnWc1Qt5NtvYSaNcOP+u1T9qYdvdihz0PPSiiqn/+/3e7Jo4EaG7TubfWGUrMQ=="
        crossorigin="anonymous" referrerpolicy="no-referrer"></script>
<script src="http://localhost/php-video-call/asset/js/client.js"></script>
</body>
</html>